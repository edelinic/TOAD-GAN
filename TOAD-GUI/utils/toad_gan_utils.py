...
def generate_sample(generators, noise_maps, reals, noise_amplitudes, num_layer, token_list, in_states=None, scale_v=1.0,
    ...
    for G, Z_opt, noise_amp in zip(generators, noise_maps, noise_amplitudes):
        ...
        I_curr = G(z_in.detach(), I_prev, temperature=0.5)

        # if current_scale == 0:
        #     cap_seed_img = plt.imread("utils/multi_token_input/path.png")
        #     cap_seed = torch.Tensor(1 - cap_seed_img[:, :, 0])
        #     cap_seed = cap_seed.unsqueeze(0).unsqueeze(0) #change shape from [h, w] to [1, 1, h, w]       

        #     for token in (token_list):
        #         if token == 'o':  
        #             tmp = cap_seed.clone().to(device) 
